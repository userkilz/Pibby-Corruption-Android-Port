...
function onUpdate(elapsed)
	if on then
		...
		setProperty('camHUD.zoom', lerp(getProperty('camHUD.zoom'), 1.01, elapsed))
		setProperty('camGame.zoom', lerp(getProperty('camGame.zoom'), getProperty('defaultCamZoom') * 1.2, elapsed))
